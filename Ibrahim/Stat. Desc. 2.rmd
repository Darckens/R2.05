...
Nous pensons que Rick fera décroitre la densité de zombie quand la quantité de bactéries Y (en UFC/mL) sera nulle. On cherche donc à déterminer le temps nécessaire pour que la quantité de bactéries soit nulle. Pour cela, nous allons résoudre l'équation $y = 0$ pour chacun des modèles.
...
La fonction \( y(x) = 1{,}034294 \cdot e^{-0{,}2093764x} \) tend vers zéro quand \( x \to +\infty \), mais ne l’atteint jamais. Il s’agit donc d’une asymptote horizontale en \( y = 0 \), ce qui explique l’absence de solution réelle à l’équation \( y(x) = 0 \).
...
$$
\Longleftrightarrow x \simeq 10.25576
$$

```{r echo=FALSE}

cat("X linéaire =", (-0.856489)/(-0.083513) )
...
Pour le modèle linéaire, la quantité de bactéries atteindra zéro après $\simeq$ 11 jours. Cela signifie que Rick devra attendre $\simeq$ 11 jours après l’injection d’une dose de 8 mL de sérum pour que la densité de zombies diminue assez pour qu'il sauve le monde.
