...
var_X_pearson <- var(data$Y) * (nrow(data) - 1) / nrow(data)

cat("Moyenne de la quantité de bactéries : ", mean(data$Y), "\n")
cat("Variance de la quantité de bactéries : ", var_X_pearson)
...
cat("Covariance entre X et Y : ", cov(data$X, data$Y, method = "pearson"))
...
$$
\hat{a} = \frac{\text{Cov}(X, Y)}{\sigma^2} ~~~ \text{et} ~~~ \bar{b} = \bar{y} - \hat{a}x
$$
...
covariance <- cov(data$X, data$Y, method = "pearson")
...
ce qui correspond au modèle exponentiel ajusté que nous retrouvons dans R après estimation des coefficients.
...
y_log <- log(data$Y)
covariance_log <- cov(data$X, y_log, method = "pearson")
a_log <- covariance_log / var(data$X)
b_log <- mean(y_log) - a_log * mean(data$X)
...
On sait que : $$R^2 = \frac{SCE}{SCT}$$ 

Pour allé plus vite, utilisons R :
